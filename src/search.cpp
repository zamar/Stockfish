...
namespace {
  ...
  template <bool PvNode> inline Depth reduction(Depth d, int mn, bool cut) {

    return (Depth) Reductions[PvNode + (cut ? 2 : 0)][std::min(int(d) / ONE_PLY, 63)][std::min(mn, 63)];
  }
...
void Search::init() {
  ...
  for (hd = 1; hd < 64; hd++) for (mc = 1; mc < 64; mc++)
  {
      ...
      double cutRed   = log(double(hd)) * log(double(mc)) / 1.5;
...
moves_loop: // When in check and at SpNode search starts from here
    ...
    while ((move = mp.next_move<SpNode>()) != MOVE_NONE)
    {
          ...
          &&  bestValue > VALUE_MATED_IN_MAX_PLY)
      {
          ...
          Depth predictedDepth = newDepth - reduction<PvNode>(depth, moveCount, false);
      ...
      if (    depth > 3 * ONE_PLY
          ...
          &&  move != ss->killers[1])
      {
          ss->reduction = reduction<PvNode>(depth, moveCount, !PvNode && cutNode);
